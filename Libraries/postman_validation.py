...
global space,reqMainkey,reqRespkey
reqMainkey = ''
space = "    "
yamlFile = ''
find_List = []
	...
	for api in yamlContent['paths']:
		for method in yamlContent['paths'][api]:
			...
			for response in yamlContent['paths'][api][method]['responses']:
				if response == 200 or response == 201 or response == 204:
...
def writePostmanCollection(Tag,dataType,subDataType,yamlFile,jsonfile,response_list):#This will add the 'Tests' and 'Pre-Request' part in postman file
	...
	for content in jsonContent['requests']:
		# reqMainkey = response_list['main_Key']
		# reqRespkey = response_list['response_key']
		...
		for element in response_list:
			...
			reverseURL = ''
			reverse = ''
			if '404' in content['name']:
				reverse = url
				if '?' in reverse:
					reverse = reverse.split('?')[0]
				# print(url)
				if '/:' in reverse:
					reverse = reverse.split('/:')[0]
				else:
				...
				# reverseURL = reverseURL.replace(reverse,'')

			if '412' in content['name']:
				api = api.rsplit('{',1)[0]

			if '{' not in api and '/:' not in url:
				if '?' in url:
					url1 = url.split('?')[0]
				else:
					url1 = url
				url1 = url1.replace(reverseURL,'')
				url1 = url1.replace('s','')
				api = api.replace('s','')
				crctCheck = url1.split(api)[-1]
				if api in url1 and element['method'].upper() == content['method'] and crctCheck == '':
					reqMainkey = element['main_Key']
					reqRespkey = element['response_key']


			elif '{' in api and '/:' in url:
				api = api.replace('{',':')
				api = api.replace('}','')
				if '/' in api[-1]:
					api = api[:-1]
		...
		# print(content['method'],reqMainkey,reqRespkey)	
		...
		if content['method'] == 'POST':
			...
			for rawElement in final_List:
				if '.' in rawElement:
					...
					content['events'][eventCheck]['script']['exec'].append(space+"pm.environment.set("+'"'+rawElement+'"'+",currentData."+rawElement+");")

		else:
			for rawElement in raw_List:
				if '.' in rawElement:
					data = rawElement.split('.')[-1]
					if reqMainkey in rawElement:
						firstRaw = rawElement.split('.')[1]
					else:
						firstRaw = rawElement.split('.')[0]
					arrayRaw = rawElement.split('.')
					arrayRaw = arrayRaw[1:(len(arrayRaw)-1)]
					firstRawDT = pre_dataType(firstRaw,reqMainkey,reqRespkey)
					if firstRawDT == 'array':
						rawElement = rawElement.replace(firstRaw,firstRaw+'[0]')
					else:
						pass

					if reqMainkey in rawElement:
						mergeElement = rawElement
					else:
						mergeElement = reqMainkey+'.'+rawElement
					content['events'][eventCheck]['script']['exec'].append(space+"pm.environment.set("+'"'+ data +'"'+',currentData.'+mergeElement+');')
				else:
					if reqMainkey in rawElement:
						mergeElement = rawElement
					else:
						mergeElement = reqMainkey+'.'+rawElement
					content['events'][eventCheck]['script']['exec'].append(space+"pm.environment.set("+'"'+rawElement+'"'+",currentData."+mergeElement+");")
...
def pre_dataType(key,Header_key,response_key):
	...
	if Header_key in yamlContent['definitions']:
		pass
	elif Header_key[0].upper()+Header_key[1:] in yamlContent['definitions']:
		Header_key = Header_key[0].upper()+Header_key[1:]


	if Header_key in yamlContent['definitions']:
		if 'properties' in yamlContent['definitions'][Header_key]:
			if key in yamlContent['definitions'][Header_key]['properties']:
				keyFinder = yamlContent['definitions'][Header_key]['properties'][key]['$ref'].split('/')[-1]
				if keyFinder in yamlContent['definitions']:
					if 'type' in yamlContent['definitions'][keyFinder]:
						return yamlContent['definitions'][keyFinder]['type']
					else:
						print('First')
				else:
					print('second')
				...
				pass
				# print('Three',key,Header_key,response_key)
